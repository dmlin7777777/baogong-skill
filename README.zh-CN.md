...
### v3.0（当前）— 多智能体 + 渲染管线 + Darwin 优化
- **Darwin Skill 9 维评估**：74.3 → **84.6**（+10.3）
- **9 处 STOP/CHECKPOINT 标记**，防止 Agent 自主失控
- **反模式清单**（8 条禁令）— 禁止捏造、禁止自我审查、禁止跳过用户确认
- **自动检测简历路径**：Glob 搜索 `resume_master.md` + 故事库路径解析
- **双列错误处理**：主方案 + 兜底方案，覆盖全部 8 种异常
